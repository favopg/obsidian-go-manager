            ...
            callback: async () => {
                ...
                if (file) {
                    ...
                    const content = [
                        ...
                        'const FUSEKI_MOVES_LIMIT = 30; // 先30手以内に含まれていれば件数あり',
                        '',
                        ...
                        '        if (c.length === 2) {',
                        '          const pt = toXY(c);',
                        '          if (pt) { moveCoords.push(pt); count++; }',
                        '        }',
                        '      }',
                        '    } catch (_) {}',
                        '',
                        ...
                        '    // 布石（名前ごと）該当の勝敗カウント（設定座標が先30手以内に含まれる場合のみ集計）',
                        '    for (const g of FUSEKI_GROUPS) {',
                        '      const has = g.coords.some(c => moveCoords.some(pt => pt.x === c.x && pt.y === c.y));',
                        '      if (has) {',

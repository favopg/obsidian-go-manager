            ...
            callback: async () => {
                ...
                if (file) {
                    ...
                    const content = [
                        ...
                        '      const checkMatch = (color, coords) =>',
                        '        coords.every(c => moves.some(m => m.color === color && m.x === c.x && m.y === c.y));',
                        '',
                        '      const mirroredCoords = g.coords.map(c => ({ x: FILTER_SZ + 1 - c.x, y: c.y }));',
                        '',
                        '      if (checkMatch("B", g.coords) || checkMatch("W", g.coords) ||',
                        '          checkMatch("B", mirroredCoords) || checkMatch("W", mirroredCoords)) {',
                        ...
                        '          }',
                        '          ',
                        '          // このノード内にコメント C[...] があるかチェック',
                        '          let j = i + 1;',
                        '          let foundComment = false;',
                        '          while (j < sgf.length && sgf[j] !== ";" && sgf[j] !== "(" && sgf[j] !== ")") {',
                        '            if (sgf.slice(j, j + 2) === "C[") {',
                        '              foundComment = true;',
                        '              break;',
                        '            }',
                        '            if (sgf[j] === "[") {',
                        '              const endIdx = sgf.indexOf("]", j);',
                        '              if (endIdx !== -1) {',
                        '                j = endIdx + 1;',
                        '              } else {',
                        '                j++;',
                        ...
                        '           const moveNum = stack[stack.length - 1];',
                        '           if (!moves.includes(moveNum)) {',
                        '             moves.push(moveNum);',
                        '           }',
                        '        }',
                        '      }',
                        ...
                        '',
                        '  // ソート対象（対局者）入力',
                        '  const hasPlayerCol = COLUMNS.includes("黒番") || COLUMNS.includes("白番");',
                        ...
                        '',
                        '  const updateDisplay = () => {',
                        '    const kw = playerInput ? (playerInput.value || "").trim().toLowerCase() : "";',
                        '    const haFilter = haSelect ? haSelect.value : "すべて";',
                        '',
                        '    // 1. フィルタリング',
                        '    let filtered = allData;',
                        '    if (haSelect && haFilter !== "すべて") {',
                        ...
                        '',
                        '    const hasResultCol = COLUMNS.includes("結果");',
                        '',
                        ...
                        '',
                        '  // イベント登録',
                        '  if (playerInput) playerInput.addEventListener("input", () => { currentPage = 1; updateDisplay(); });',
                        '  if (haSelect) haSelect.addEventListener("change", () => { currentPage = 1; updateDisplay(); });',

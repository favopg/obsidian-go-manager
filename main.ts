            ...
            callback: async () => {
                ...
                if (file) {
                    ...
                    const content = [
                        ...
                        '    // 布石（名前ごと）該当の勝敗カウント と 行追加用フラグ',
                        '    // 要件: 「黒 or 白が指定座標すべてを同色で打っている場合のみ」カウント/一致',
                        '    let matchesAnyFuseki = false;',
                        '    for (const g of FUSEKI_GROUPS) {',
                        ...
                        '    // テーブル一覧の条件: 布石設定がある場合は一致したもののみ、未設定なら全件',
                        '    if (FUSEKI_GROUPS.length > 0) {',
                        '      if (matchesAnyFuseki) rows.push([pb, pw, gn, ha, re, link]);',
                        '    } else {',
                        ...
                        '    const pageRows = base.slice(start, end);',
                        '    // テーブル描画',
                        ...
                        '  render("");',
                        '  // 入力イベントで再描画',
                        '  input.addEventListener("input", () => { currentPage = 1; render(input.value); });',
                        '  prevBtn.addEventListener("click", () => { if (currentPage > 1) { currentPage--; render(input.value); } });',
